...
export const chatWithAssistant = async (req, res, next) => {
  try {
    ...
    if (intent.intent === "tutoring")
      return res.json({ type: "text", reply: intents.buildTutoringReply() });
    if (intent.intent === "cafe")
      return res.json({ type: "text", reply: intents.buildCafeReply() });
...
export const addChatJobController = async (req, res, next) => {
  try {
    ...
    const parsedPriority = Number.isFinite(Number(priority))
      ? Number(priority)
      : 5;
    ...
    // Persist job metadata so status polling can still resolve after queue cleanup.
    await ChatLog.create({
      ...
      console.error(
        "[chatController] Failed to persist queued chat log:",
        err.message,
      );
    });
    ...
    return res.status(202).json({
      ...
      jobId: String(job.id),
      message:
        "Your message has been queued. Poll /api/chat/status/:jobId for the result.",
    });
  } catch (error) {
...
export const getJobStatusController = async (req, res, next) => {
  try {
    ...
    if (job) {
      ...
      let error = job.failedReason || job.data?.failureReason || null;

      if (!result && state === "completed") {
        const chatLog = await ChatLog.findOne({ jobId });
        result = chatLog?.reply
          ? {
              type: chatLog.type || "text",
              reply: chatLog.reply,
              products: chatLog.products || [],
              priceBand: chatLog.priceBand || null,
            }
          : null;
        error = error || chatLog?.errorMessage || null;
      }
    ...
    return res.json({
      ...
      result:
        chatLog.status === "completed"
          ...
          : null,
      error: chatLog.errorMessage || null,
      attemptsMade: chatLog.attemptsMade || 0,
